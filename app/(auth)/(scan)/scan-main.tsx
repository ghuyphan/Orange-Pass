...
export default function ScanScreen() { 
  ...
  const { device, hasPermission, torch, toggleFlash } = useCameraSetup(cameraRef, setupCamera);

  // Use effect to defer camera setup
  useEffect(() => {
    // Delay camera setup to improve navigation performance
  ...
  // Sheet handler
  const onOpenSheet = useCallback((type) => { 
    setSheetType(type); 
    bottomSheetRef.current?.snapToIndex(0); 
  }, []); 
          ...
          <Reanimated.View onLayout={onLayout} style={[StyleSheet.absoluteFill, animatedCameraStyle]}> 
            {isCameraReady && ( 
              <ReanimatedCamera 
                ...
                androidCameraPreviewToUse="analyzer"
              /> 
            )} 
            {isCameraReady && ( 
              <>
