...
export const ThemedFAB = forwardRef<View, ThemedFABProps>(
  (
    ...
    useEffect(() => {
      ...
      // Clear timeout if component unmounts or 'open' changes before timeout fires
      return () => {
    ...
    // useEffect to clear all timeouts on unmount
    useEffect(() => {
      return () => {
        ...
        // No need to clear closingTimeoutRef here again as it's handled in its own useEffect
      };
    ...
    const handleFABPress = () => {
      ...
      if (animationTimeoutRef.current) {
        clearTimeout(animationTimeoutRef.current); // Clear previous timeout if any
      }
    ...
    const handlePressWithAnimation = (onPress: () => void) => () => {
      ...
      if (pressTimeoutRef.current) {
        clearTimeout(pressTimeoutRef.current); // Clear previous timeout if any
      }
