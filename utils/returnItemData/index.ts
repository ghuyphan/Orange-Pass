...
import vietQRBanksData from '@/assets/vietQRBanks.json';
...
export const returnItemData = (code: string, type?: DataType): ItemDataWithType =>
  dataManager.getItemData(code, type);

export const returnItemsByType = (type: DataType) => dataManager.getItemsByType(type);

export const returnItemCode = (searchTerm: string, type?: DataType) =>
  dataManager.searchItems(searchTerm, type ? [type] : undefined);

export const returnItemCodeByBin = (bin: string) => dataManager.getItemCodeByBin(bin);
