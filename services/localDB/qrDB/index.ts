...
export async function syncQrCodes(userId: string) {
    ...
    try {
      ...
      // Handle sync operations concurrently for each unsynced QR code
      const promises = unsyncedQrCodes.map(async (qrCode) => {
        try {
            console.log(`Syncing QR code ${qrCode.id}`);
          const data = {
            code: qrCode.code,
